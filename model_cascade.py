...
from faster_rcnn.utils.tf_utils import pad_to_fixed_size
...
######################################################################################################################
        ...
        # 检测网络
        rcnn_deltas2, rcnn_class_logits2 = rcnn2(features, train_rois2, config.NUM_CLASSES, config.IMAGE_MAX_DIM,
                                              config.head_fn, pool_size=config.POOL_SIZE,
                                              fc_layers_size=config.RCNN_FC_LAYERS_SIZE)
...
#####################################################################################################################
        ...
        cls_loss2_rcnn = Lambda(lambda x: detect_cls_loss(*x), name="rcnn2_class_loss")(
        ...
        model = Model(inputs=[input_image, input_image_meta, gt_class_ids, gt_boxes],
                      outputs=[cls_loss_rpn, regress_loss_rpn, regress_loss_rcnn, cls_loss_rcnn,bbox_loss2_rcnn,cls_loss2_rcnn] + [
...
########################################################################################################################
        ...
        rcnn_deltas2, rcnn_class_logits2 = rcnn2(features, detect_boxes, config.NUM_CLASSES, config.IMAGE_MAX_DIM,
                                                 config.head_fn, pool_size=config.POOL_SIZE,
                                                 fc_layers_size=config.RCNN_FC_LAYERS_SIZE)
...
def rcnn2(base_layers, rois, num_classes, image_max_dim, head_fn, pool_size=(7, 7), fc_layers_size=1024, l2_reg=5e-4):
    # RoiAlign
    x = RoiAlign(image_max_dim, pool_size=pool_size)([base_layers, rois], name= 'RoiAlign2')  #
    # 收缩维度
