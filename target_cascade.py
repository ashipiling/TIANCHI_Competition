...
def rpn_targets_graph(gt_boxes, gt_cls, anchors, anchors_tag, rpn_train_anchors=None):
    ...
    # 根据正负样本比1:1,确定最终的正样本
    positive_num = tf.cond(tf.greater(tf.shape(gt_cls)[0], 0),
                   true_fn=lambda: tf.minimum(tf.shape(positive_anchor_indices)[0], int(rpn_train_anchors * 0.9)),
                   false_fn=lambda: tf.shape(gt_cls)[0])
    ...
    negative_indices = tf.random.shuffle(negative_indices)[:negative_num]
...
def shuffle_sample(tensor_list, tensor_size, sample_size):
    ...
    :return:
    """
    shuffle_indices = tf.random.shuffle(tf.range(tensor_size))[:sample_size]
...
def detect_targets_graph(gt_boxes, gt_class_ids, proposals, train_rois_per_image, roi_positive_ratio):
    ...
    proposal_neg_idx = tf.cond(  # 需要考虑GT个数为0的情况;全部都是负样本
        tf.greater(tf.shape(gt_class_ids)[0], 0),
    ...
    proposal_neg_idx = tf.random.shuffle(proposal_neg_idx)[:negative_num]
    # 收集负样本
...
def detect_targets_graph2(gt_boxes, gt_class_ids, proposals, train_rois_per_image, roi_positive_ratio):
    """
    每个图像生成检测网络的分类和回归目标
    IoU>=0.5的为正样本；IoU<0.5的为负样本
    :param gt_boxes: GT 边框坐标 [MAX_GT_BOXs, (y1,x1,y2,x2,tag)] ,tag=0 为padding
    ...
    proposal_neg_idx = tf.cond(  # 需要考虑GT个数为0的情况;全部都是负样本
        tf.greater(tf.shape(gt_class_ids)[0], 0),
        true_fn=lambda: tf.where(tf.logical_and(proposal_iou_max < 0.6, proposal_iou_max > 0.1))[:, 0],
        false_fn=lambda: tf.cast(tf.range(proposals_num), dtype=tf.int64)
    )
    # 确定负样本数量
    negative_num = tf.minimum(train_rois_per_image - positive_num, tf.shape(proposal_neg_idx)[0])
...
class DetectTarget2(keras.layers.Layer):
    """
    检测网络分类和回归目标;同时还要过滤训练的proposals
    """
    ...
    def __init__(self, batch_size, train_rois_per_image=200, roi_positive_ratio=0.33, **kwargs):
        ...
        self.train_rois_per_image = train_rois_per_image
        self.roi_positive_ratio = roi_positive_ratio
        super(DetectTarget2, self).__init__(**kwargs)
        # print("roi_positive_ratio：{}".format(roi_positive_ratio))
    ...
    def call(self, inputs, **kwargs):
        ...
        #                    dtype=[tf.float32] * 4)
        outputs = tf_utils.batch_slice([gt_boxes, gt_class_ids, proposals],
                                       lambda x, y, z: detect_targets_graph2(x, y, z, self.train_rois_per_image,
                                                                            self.roi_positive_ratio),
                                       self.batch_size)
        return outputs
...
def refine_detections_graph2(rois, probs, deltas, max_output_size_2):
    ...
    output_rois = tf_utils.pad_to_fixed_size(refined_rois, max_output_size_2)
    output_rois.set_shape([max_output_size_2, 5])

    return output_rois

class DetectionLayer2(keras.layers.Layer):
    def __init__(self, batch_size,max_output_size_2 = 40, **kwargs):
        super(DetectionLayer2, self).__init__(**kwargs)
        self.batch_size = batch_size
        self.max_output_size_2 = max_output_size_2

    def call(self, inputs, **kwargs):
        rois = inputs[0][..., :-1]
        #rcnn_class = TimeDistributed(Activation("softmax"),name="rcnn_class")(inputs[1])
        rcnn_class = tf.nn.softmax(logits=inputs[1], axis=-1)
        rcnn_bbox = inputs[2]

        refined_rois = tf_utils.batch_slice(
            [rois, rcnn_class, rcnn_bbox],
            lambda x, y, w: refine_detections_graph2(x, y, w, self.max_output_size_2),
            self.batch_size)

        return refined_rois

    def compute_output_shape(self, input_shape):
        return (input_shape[0][0], self.max_output_size_2, 4 + 1)
